...
#http            = :5000   #单独起uwsgi 就注释下面的  
socket			= /root/qfxfz/qfxfz.sock  #起了nginx 就注释上面的
# 设置socket的权限
chmod-socket    = 666 
# 退出的时候是否清理环境
vacuum          = true
...
在`/etc/nginx/conf.d`目录下，新建一个文件，叫做qfxfz.conf`，然后将以下代码粘贴进去：
